...
* 使用 **线程池 + 非阻塞socket + epoll(ET)** 的模拟Proactor并发模型
...
压力测试
-------------
在关闭日志后，使用Webbench对服务器进行压力测试，对listenfd和connfd均采用ET模式，可实现上万的并发连接，下面是测试结果. 
...
```bash
./server [-l LOGWrite] [-o OPT_LINGER] [-s sql_num] [-t thread_num] [-c close_log]
```
