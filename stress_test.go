...
func main() {
	// 1. 命令行参数解析
	concurrency := flag.Int("c", 1000, "并发连接数 (Concurrent connections)")
	durationSec := flag.Int("t", 10, "测试持续时间/秒 (Test duration in seconds)")
	targetURL := flag.String("url", "http://192.168.19.128:9006/", "目标服务器 URL")
	flag.Parse()
	...
	for i := 0; i < *concurrency; i++ {
		...
		go func() {
			...
			for {
				...
				req, _ := http.NewRequestWithContext(ctx, "POST", *targetURL, bytes.NewBuffer(jsonData))
